// Mithriv Home Page Interactive Scripts
window.runConsoleSimulation = () => {
        ...
        `).join('');
            ...
            function sendMessage() {
                ...
                setTimeout(() => {
                    ...
                    if (userText.toLowerCase().includes("fire")) {
                        const node = chatNodes['initial_fire'];
                        ...
                        const node = chatNodes['initial_visitor'];
                        let completeHTML = (node.visualHTML || "") + renderOptions(node.options);
                        addMessageObj(false, node.aiResponse, completeHTML);
                    } else if (userText.toLowerCase().includes("access")) {
                        const node = chatNodes['initial_access'];
                        let completeHTML = (node.visualHTML || "") + renderOptions(node.options);
                        addMessageObj(false, node.aiResponse, completeHTML);
                    } else {
            ...
            if (window.consoleClickSimulationListener) {
                document.removeEventListener('click', window.consoleClickSimulationListener);
            }
            window.consoleClickSimulationListener = function (e) {
                if (commandPalette && !commandPalette.contains(e.target) && e.target.id !== 'commandBtn' && !e.target.closest('#commandBtn')) {
                    commandPalette.classList.remove('active');
                    if (commandBtn) commandBtn.classList.remove('active');
                }
            };
            document.addEventListener('click', window.consoleClickSimulationListener);
            ...
            function addMessageObj(isUser, text, htmlContent = null) {
                ...
                bubbleContainer.className = isUser ? "chat-bubble bubble-user" : "chat-bubble bubble-ai";

                let avatarHtml = isUser ? `<div class="bubble-avatar user-avatar">U</div>` : `<div class="bubble-avatar">M</div>`;
    ...
    `;
            ...
            window.handleOption = function (nextNodeId, btnEl) {
                if (btnEl) {
                    btnEl.parentElement.querySelectorAll('button').forEach(b => b.disabled = true);
                    addMessageObj(true, btnEl.innerText);
                }

                const node = chatNodes[nextNodeId];
                if (!node) return;
...
window.runDotCanvas = () => {
                ...
                (function () {
                    ...
                    let lastFrame = 0;
                    function draw(ts) {
                        window.dotCanvasAnimId = requestAnimationFrame(draw);
                    ...
                    window.dotCanvasResizeObserver = ro;
                    resize();
                    window.dotCanvasAnimId = requestAnimationFrame(draw);
                })();

          
window.cancelDotCanvasAnim = () => { 
    if (window.dotCanvasAnimId) cancelAnimationFrame(window.dotCanvasAnimId); 
    if (window.dotCanvasResizeObserver) {
        window.dotCanvasResizeObserver.disconnect();
        window.dotCanvasResizeObserver = null;
    }
};

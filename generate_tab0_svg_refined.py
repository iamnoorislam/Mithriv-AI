...
<svg width="100%" height="100%" viewBox="0 0 400 250" style={{ position: 'relative', zIndex: 1 }}>
  <defs>
    <style>
      {`
        ...
        @keyframes pulse-node-blue {
          0%, 100% { transform: scale(1); opacity: 0.8; fill: #4993E3; }
          50% { transform: scale(1.5); opacity: 0.2; fill: #4993E3; }
        }
        @keyframes pulse-node-gold {
          0%, 100% { transform: scale(1); opacity: 0.8; fill: #E7C73B; }
          50% { transform: scale(1.5); opacity: 0.2; fill: #E7C73B; }
        }
        @keyframes pulse-node-green {
          0%, 100% { transform: scale(1); opacity: 0.8; fill: #AFF962; }
          50% { transform: scale(1.5); opacity: 0.2; fill: #AFF962; }
        }
    ...
    <filter id="glowGold"><feGaussianBlur stdDeviation="3" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
  ...
  <!-- A (Blue): (150, 170) -> (150, 110) -->
  <line x1="150" y1="170" x2="150" y2="110" stroke="#4993E3" strokeWidth="1.5" strokeDasharray="4 4" style={{ animation: 'dash-flow 1s linear infinite reverse' }} />
  <!-- B (Gold): (260, 185) -> (260, 125) -->
  <line x1="260" y1="185" x2="260" y2="125" stroke="#E7C73B" strokeWidth="1.5" strokeDasharray="4 4" style={{ animation: 'dash-flow 1s linear infinite reverse' }} />
  <!-- C (Green): (210, 215) -> (210, 155) -->
  <line x1="210" y1="215" x2="210" y2="155" stroke="#AFF962" strokeWidth="1.5" strokeDasharray="4 4" style={{ animation: 'dash-flow 1s linear infinite reverse' }} />

  <!-- BEAMS from Layer 2 to Core (Red/Pinkish) -->
  ...
  <g transform="translate(200, 180)">
    ...
    <circle cx="-10" cy="-30" r="1.5" fill="rgba(234, 73, 178, 0.3)" />
    
    <!-- Threat Source Dots -->
    <!-- Node A (Blue): -50, -10 -->
    <circle cx="-50" cy="-10" r="3" fill="#4993E3" filter="url(#glowBlue)" />
    <text x="-60" y="-8" fill="rgba(255,255,255,0.6)" fontSize="6" fontFamily="var(--font-mono)">IDENTITY_LOG</text>
    
    <!-- Node B (Gold): 60, 5 -->
    <circle cx="60" cy="5" r="3" fill="#E7C73B" filter="url(#glowGold)" />
    <text x="70" y="7" fill="rgba(255,255,255,0.6)" fontSize="6" fontFamily="var(--font-mono)">FW_DENY</text>

    <!-- Node C (Light Green): 10, 35 -->
    <circle cx="10" cy="35" r="3" fill="#AFF962" filter="url(#glowGreen)" />
  ...
  <g transform="translate(200, 120)">
    <!-- Plane Base -->
    <g transform="scale(1, 0.5) rotate(45)">
      <rect x="-70" y="-70" width="140" height="140" fill="rgba(234, 73, 178, 0.02)" stroke="rgba(234, 73, 178, 0.3)" strokeWidth="1" rx="4" />
    </g>
    
    <!-- Correlated Triangle -->
    <polygon points="-50,-10 60,5 10,35" fill="rgba(228, 72, 86, 0.05)" stroke="#E44856" strokeWidth="1.5" strokeDasharray="3 3" />
    
    <!-- Pulsing nodes at vertices -->
    <circle cx="-50" cy="-10" r="4" style={{ transformOrigin: '-50px -10px', animation: 'pulse-node-blue 2s infinite' }} />
    <circle cx="60" cy="5" r="4" style={{ transformOrigin: '60px 5px', animation: 'pulse-node-gold 2s infinite 0.6s' }} />
    <circle cx="10" cy="35" r="4" style={{ transformOrigin: '10px 35px', animation: 'pulse-node-green 2s infinite 1.2s' }} />
...
print("Refined Tab 0 SVG with full palette.")
